...
| **T-0612-04** | DEPLOY | **P0** | … | **FIXED** @ `e167257` · 双路径不可用 → 200 + 明确文案（非 500） |
| **T-0612-06** | DEPLOY | P2 | … | **FIXED** @ `848e116`（Java）+ Python `parser.py` |
...
ref: round-2026-06-12-qa-regression
verdict: APPROVED
summary: e167257 第三降级路径 — 双不可用不再 500；T-0612-01～04/06 代码侧通过

- `MultiTurnController` 路径 3：`ApiResponse.ok` + 用户可见 fallback 文案（HTTP 200，符合 §1.2「或 503 明文」）
- T-0612-01～03 维持 FIXED；T-0612-05 保持 ESCALATED → upgrade plan
- **遗留**：125 部署 qa-agent 后业务 2 问均真 200 待 Operator Co-test（非代码阻塞）
...
ref: case
verdict: CLOSED
archive: test-to-settle/done/round-2026-06-12-qa-regression.md
summary: 0612 Co-test 编译+多轮 QA 回归 case 关闭
...
- [x] T-0612-04 / T-0612-06 已 FIX + Reviewer APPROVED（或 ESCALATED 有去向）
- [x] T-0612-01～03 标 FIXED 已核对
- [x] T-0612-05 已在 upgrade plan 路由，本表保持 ESCALATED
- [x] 已有 **`Reviewer` + `verdict: CLOSED`** 块
- [x] `git mv` → [`done/`](done/README.md) · [`TASKS.md`](../TASKS.md) 删行
