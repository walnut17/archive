...
§1 任务描述（Bug 表，非 block）
  ...
  → Coder ↔ Reviewer
  → Reviewer(CLOSED)（审查员，必）
  → done/ + TASKS 删行
         ↘ Analyst ESCALATED → complexity.md → plan + TASKS UPGRADE
```

**关 case 条件**：各 `T-*` 已 APPROVED / ESCALATED / WONTFIX，且有 **`role: Reviewer(CLOSED)`** 块。
...
| 实现与审查 | plan **§3 Agent Blocks**：Coder ↔ Reviewer → **Reviewer(CLOSED)** |
...
```text
§0～§2 定稿 → §3 Agent Blocks（Coder ↔ Reviewer → Reviewer(CLOSED)）→ done/ → TASKS 删行
```
...
```text
TASKS（待审）→ Case 文件 Reviewer 块 → 全过 → Reviewer(CLOSED) → done/ → 删 TASKS 行
```
...
> **与 Case 流程配合**，但不替代 Recorder/Analyst/Coder/Reviewer/Reviewer(CLOSED)。
...
Guide 出 Step N → Operator 执行 → 反馈结果
        ...
        └─ bug  → Guide 起草 case §1 行 → Operator 或 Recorder 写入 test-to-settle
                      → 后续 Analyst / Coder / Reviewer / Reviewer(CLOSED)（非 co-test 当场硬修，除非用户授权）
```
...
| **代码审查员** | **Reviewer** block（含 **CLOSED** 关单）· [`CODE-REVIEWER.md`](CODE-REVIEWER.md) | CLOSED → **done/** · TASKS 删行 |
...
```text
              ...
              部署/联调     │                     │ FAIL
                    ...
                    └──────────────┘      │ Agent Blocks │
                           │              └──────┬───────┘
                           │                     │
                           │ 发现 bug            │ Coder ↔ Reviewer → Reviewer(CLOSED)
                           └──────────┬──────────┘
                                      ▼
                              ┌──────────────┐
                              │ test-to-settle/      │
